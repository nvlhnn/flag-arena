...
test('audio controls persist, validate input and deliver overlay test events',async()=>{
 ...
 assert.deepEqual(JSON.parse(readFileSync(join(directory,'state.json'),'utf8')).audio,settings);
