...
import {acceptVote,catchUpBonus,catchUpMultiplier,initialState} from '../lib/arena.ts';
...
test('catch-up thresholds, activation, and off switch',()=>{
 ...
 assert.equal(catchUpBonus({...initialState(),scores:{ID:10000,BR:0}},'BR'),10);
 assert.equal(catchUpBonus({...initialState(),scores:{ID:100000,BR:0}},'BR'),100);
 assert.equal(catchUpBonus({...initialState(),scores:{ID:10000,BR:9500}},'BR'),0);
});
...
test('bonuses respect level progression and recalculate after crossing a threshold',()=>{
 ...
 assert.equal(first.scores.BR,1017);
 ...
 assert.equal(second.recent[0].points,14);
...
test('level-up still fires when bonus award exceeds new viewer level',()=>{
 ...
 assert.equal(result.recent[0].points,9);
