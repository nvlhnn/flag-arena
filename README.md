...
- New connections skip initial chat history. Previously connected videos restore their saved session. For a new URL, choose **Continue previous session** to carry progress forward or **Start fresh** to start at zero.
...
The first successful subscriber snapshot is a baseline, not a bonus event. Later checks run every 15 seconds while matching live chat is connected. Each cycle checks the newest 50 subscribers, then advances at most one saved catch-up page. Each successful page and its pending cursors are committed to SQLite immediately. Catch-up continues across cycles and restarts of the same livestream without a 500-entry hard stop. Choosing Start fresh for a new livestream clears known subscriber IDs, pending bonuses, and catch-up cursors and takes a fresh baseline. Continue previous session preserves them. Google sign-in is retained. Subscriber ledgers are saved per arena session; previously connected URLs restore their session ledger. Legacy channel-wide ledgers without a stream ID are reset once on upgrade. Mixed pages keep scanning; fully known or older pages finish a scan. Rejected cursors trigger a recovery scan, and temporary network/rate-limit errors retry automatically. Real quota exhaustion and authorization errors still pause tracking. API availability and public subscription visibility can limit detection. A subscriber channel ID gets at most one detected bonus in the current livestream ledger, preventing repeated unsubscribe/resubscribe or reconnection bonuses.

A new subscriber earns **100 points** for their latest country vote in the open match. With no country selected, the bonus waits for their next country message in that same match; it expires when that round ends. Closed results are never updated by delayed subscriber detections. The alert slides from the right with their name, country flag, and awarded points; unassigned or closed-match subscriptions show thanks without claiming a bonus. English audio says “Thank you for subscribing,” followed by the country bonus when awarded. Countdown/results audio takes priority. The demo preview can exercise the animation and bonus without Google credentials, using demo scores only.

The previous Like +50 banner has been removed. Chat messages still add 1 point with no voting cooldown.

### Match viewer levels
Accepted country votes earn one XP at most once per five seconds per YouTube channel ID, using message timestamps. Levels 1–5 start at 0, 40, 120, 240, and 360 XP; every accepted vote scores its pre-vote level (1–5 points). The leveling vote displays the new level but the increase applies to the next vote. Level 5 takes at least 29m55s from the first XP, approximately 30 minutes. Invalid and replayed messages earn no XP. Subscriber bonuses remain a flat 100 points with no XP. Viewer progress persists across restarts and resets on every new match, including keep-scores. Bottom-strip level-up events last three seconds with a bounded queue; country popups remain name-only. Existing scores are preserved and prior votes are not awarded retroactive XP.
...
### Continuing after a failed broadcast

Keep Flag Arena running, create the replacement broadcast on the same YouTube channel, and paste its new URL into Connect YouTube. Choose **Continue previous session**, then **Connect live chat**. The old chat is replaced only after the new URL has been validated; you do not need to disconnect first. The default choice is Continue when a previous stream exists.

Scores, viewer XP/levels, country contributions, match state, and subscriber bonus history carry forward. A fresh chat checkpoint skips the replacement chat's initial history. Each broadcast keeps separate analytics, while linked URLs share the latest saved arena progress, including after restarting the app. **Start fresh** applies to a new URL and leaves earlier sessions saved; an already connected URL always restores its saved session. Match countdown deadlines retain their existing behavior and are not paused by a stream failure.

### Manual storage cleanup

Open **Settings → Storage & reset** in the local studio. Select old sessions, choose **Review cleanup**, read the included broadcasts and record counts, then confirm permanent deletion. A session includes every broadcast connected through Continue previous session. The current or last connected session is protected even in demo mode; disconnecting does not remove that protection.

Cleanup deletes the selected sessions' saved scores, viewer progress, subscriber ledgers, vote and donation history, rankings, duplicate records, and chat checkpoints in one transaction. It does not delete demo progress, audio preferences, API credentials, Google sign-in, diagnostics, or request-budget records. Cleanup creates no backup and cannot be undone. Reconnecting a deleted URL starts new tracking with initial chat history skipped. The server rejects stale previews or sessions that become current before confirmation.

Storage shows the main database's allocated size and reusable free pages, excluding temporary WAL files. Deleted space is reused by SQLite for future records; cleanup does not run a blocking database compaction or guarantee an immediate smaller file on disk. No automatic deletion or retention schedule is enabled.

### Super Chat overlay and point awards

In **Overlay → Overlay version**, choose **Current** or **Super Chat**. The choice is saved globally and shared with OBS. Super Chat uses a **9-column × 11-row** flag board with compact reward, voter, and supporter sections. The wall ranks **total USD donations per donor in the current arena session**, including linked replacement broadcasts and all rounds. Donor identity uses channel ID, not display name. The latest name/avatar is shown; a country flag appears only when the donor's assigned purchases support one country. Points show the session's awarded total. Gold, silver, and bronze styling distinguish ranks 1–3. Equal totals use first donation time, then donor ID, for stable order. Unconverted purchases are marked pending and excluded from the USD total until conversion completes. Original purchase amounts remain in donation history.

Three donor cards fit in the rail. With more than three donors, cards scroll left continuously (one card-width every nine seconds), looping in descending total order. A 12-card buffer refreshes ahead of the visible window; only five cards mount and cached cards keep moving during temporary network failures. Reduced-motion mode swaps cards without scrolling. Super Chat switches immediately even in demo mode or with no donations, showing an empty state without inventing donors. Current restores the original layout.

New Super Chats award **1,000 country points per USD**, independently of the chosen layout. Examples: $5.40 → 5,400; $3.19 → 3,190. Non-USD purchases wait for the existing stored USD reference conversion; fractional points are rounded down. Awards never earn XP, receive catch-up multipliers, or inflate chat-voter rankings. Subscriber rewards stay +100. Trailing chat votes also receive one extra point per 1,000 points behind the leader once the leader reaches 1,000, in addition to the existing catch-up multiplier. The top sections use a gold-and-mint reward header, a compact voter strip, and a supporter wall; no target is displayed.

The award uses the donation's country assignment from chat voting. On a replacement broadcast, an existing country selection in the same round can carry forward. An unassigned donation waits for a country vote in that round. Late deliveries, pending conversions, or unassigned donations cannot change finished results or award into a later round. A new round/reset expires pending awards. Existing historical donations remain visible with thanks but never receive retroactive points. Cards display conversion/country waiting status or a closed-match acknowledgement instead of claiming unawarded points.

Award records and score changes commit with chat checkpoints, so duplicate messages and restarts cannot award the same donation twice. Session cleanup removes these records with the associated donation history. YouTube avatars use the official `profile_image_url` field, with an icon fallback for unavailable images. The donation report still covers Super Chats only, not Super Stickers or other paid events. Live paid-event delivery and OBS rendering must still be verified on a monetized channel.

### Donation voice and country celebrations

In **Alerts & audio → Donations**, configure English speech, spoken donor names, spoken amounts, visual celebrations, landmarks, volume, and duration (6-15 seconds). Master mute and English announcer settings also apply. Use **Test donation celebration** in an open demo round; previews add no points or donation records. Audio plays in the open OBS overlay; both the studio and overlay show the animation.

Celebrations trigger when a new Super Chat award receives its country and converted points. They queue sequentially (up to 30 celebrations); every donation still scores if a burst exceeds this visual queue. Countdown/results suppress celebrations, and a new round clears the queue. Opening an overlay mid-celebration does not replay old speech. Short durations may shorten spoken details.

All 250 countries and territories have a standalone original SVG illustration in `public/landmarks/`, including monuments, landscapes, and local characteristics. Only the active illustration loads. Disable landmarks to use flags; a missing image also falls back to its flag. No runtime image generation is needed. Review all assets at `/landmarks/gallery.html`. Rebuild the authored geometry with `node --import tsx scripts/generate-country-artwork.ts`; the seven original assets are preserved.

Dynamic English speech uses installed Windows English voices through System.Speech, without a cloud API. Sanitized names and amount/points phrases are cached under `donation-voice` in the runtime data directory (`.arena` by default, or `ARENA_DATA_DIR`). The cache is capped at 256 clips / 32 MB and evicts older generated clips; evicted names may be synthesized again. Synthesis is serialized with up to 30 pending phrases. If synthesis is unavailable or slow, the overlay uses prerecorded thanks. `scripts/generate-announcer.ps1` also regenerates the fallback phrases. Restart the local server and refresh the OBS browser source after installing this update.

## Studio navigation

- **Live**: connect or recover chat, continue a session, manage rounds, and open the separate demo test area. A connected stream puts round controls first and collapses connection options.
- **Overlay**: choose Current/Super Chat and copy the OBS URL.
- **Alerts & audio**: Donations (voice, visuals, preview), Subscribers (reward status and demo preview), General audio (master mute, announcer, volume, samples).
- **History**: recorded stream statistics, donors, donations, and voters.
- **Settings**: YouTube API/OAuth access, scoring rules, storage cleanup/reset, and diagnostics. Subscriber account setup is separate from subscriber alert testing.

The status bar stays visible and distinguishes demo, connected live, and disconnected live states. Score reset is under Settings with confirmation; finishing a normal round stays in Live. Preview is present only in Live, Overlay, and Alerts. Audience-facing overlay behavior and data storage are unchanged by this studio reorganization.
