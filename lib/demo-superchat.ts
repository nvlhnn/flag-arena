...
export function demoSupporters(cards:DemoSuperChat[]=[]):SupporterPage{
 ...
 const entries=[...donors].map(([name,donations])=>{
  ...
  const firstCountry=donations.find(card=>card.country)?.country??null;
  return {id:name,name,avatar:'',country:firstCountry,countries:firstCountry?[firstCountry]:[],rank:0,usdMicros:String(usd),points:donations.reduce((sum,card)=>sum+card.points,0),donationCount:donations.length,pendingCount:0,amounts:[...amounts].sort(([a],[b])=>a.localeCompare(b)).map(([currency,amount])=>({currency,amountMicros:String(amount)}))};
