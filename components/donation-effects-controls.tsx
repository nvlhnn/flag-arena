...
export function DonationEffectsControls({settings,save,test,demo}:{settings:DonationEffects;save:(value:DonationEffects)=>Promise<void>;test:(country:string)=>Promise<void>;demo:boolean}){
 ...
 return <div className="control-card audio-controls"><h2>Donation celebrations</h2><p>English thanks and a country celebration when Super Chat points are awarded.</p>
  <fieldset disabled={busy} style={{border:0,padding:0,minWidth:0,display:'grid',gap:10}}>
   <h3 className="studio-control-group">Voice</h3>
   {([['voice','Donation voice'],['names','Say donor name'],['amounts','Say donation amount']] as const).map(([key,label])=><label className="audio-toggle" key={key}><input type="checkbox" checked={settings[key]} onChange={e=>void run(()=>save({...settings,[key]:e.target.checked}))}/>{label}</label>)}
   ...
   <small>Master mute, English announcer, and stream volume also apply.</small>
   <h3 className="studio-control-group">On-screen celebration</h3>
   {([['animation','Show celebration'],['landmarks','Show country landmarks']] as const).map(([key,label])=><label className="audio-toggle" key={key}><input type="checkbox" checked={settings[key]} onChange={e=>void run(()=>save({...settings,[key]:e.target.checked}))}/>{label}</label>)}
   ...
   <small>Every country has an illustration. Disable landmarks to use flags. Duration applies to new celebrations.</small>
   <a className="secondary" href="/landmarks/gallery.html" target="_blank" rel="noreferrer">Browse all country artwork</a>
   <h3 className="studio-control-group">Test in demo</h3>
