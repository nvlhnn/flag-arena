...
export function MatchOverlay({match}:{match?:Match}){
 ...
 if(match.phase==='countdown')return <div className="match-countdown" role="status"><span>FINAL SECONDS</span><div className="countdown-ring"><strong key={remaining}>{remaining}</strong></div><b>{remaining?'VOTE FOR YOUR COUNTRY':'VOTING CLOSED'}</b><small>Every vote adds 1 point</small></div>;
