...
function persist(){const temporary=file+'.tmp';writeFileSync(temporary,JSON.stringify({demo,live,video:savedVideo,resume,activeMode:state.mode}),{flush:true});if(existsSync(file)){copyFileSync(file,file+'.backup.tmp');renameSync(file+'.backup.tmp',file+'.backup');}renameSync(temporary,file);}
...
function beginStreaming(key:string,chat:string){
 ...
 void consumeChat({read:source.read,signal:current.controller.signal,diagnostic:diagnostics.record,initialPage:resume.page,beforeAttempt:()=>budget.reserve('stream'),onInvalidPage:()=>{resume={chat,since:Date.now()};persist();},
  onBatch:batch=>{if(session!==current)return;finalizeMatch();const previous=state;state=applyChatBatch(state,batch,current.since);const accepted=(Object.values(state.scores).reduce((a,b)=>a+b,0)-Object.values(previous.scores).reduce((a,b)=>a+b,0))/100;diagnostics.record({event:'votes',accepted,ignored:(batch.items?.length||0)-accepted});const changed=!!batch.nextPageToken&&batch.nextPageToken!==resume?.page;if(batch.nextPageToken)resume={chat,since:current.since,page:batch.nextPageToken};if(state!==previous)publish();else if(changed)persist();},
...
createServer(async(req,res)=>{
 try{
  ...
  if(req.method==='POST'&&url.pathname.startsWith('/api/')){
   ...
   if(url.pathname==='/api/match/next'){if(typeof body.reset!=='boolean')throw new Error('Choose reset or keep scores.');state=nextMatch(state,body.reset);if(state.mode==='live'&&resume){resume={...resume,since:state.match!.openedAt!};if(session)session.since=resume.since;}publish();json(res,200,{ok:true});return;}
