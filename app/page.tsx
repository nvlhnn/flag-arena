...
export default function Home(){
 ...
 <nav className="studio-tabs" aria-label="Studio controls">{[["live","Live"],["subscribers","Subscribers"],["audio","Audio"],["setup","OBS & health"],["analytics","Analytics"],["storage","Storage"]].map(([id,label])=><button key={id} aria-pressed={studioTab===id} aria-controls={`studio-${id}`} onClick={()=>setStudioTab(id)}>{label}</button>)}</nav>
...
<section id="studio-setup" hidden={studioTab!=="setup"} aria-label="OBS and diagnostics"> <div className="control-card"><div className="card-title"><h2>Add to OBS</h2></div><p>Add a Browser Source. Set its width to <b>1080</b> and height to <b>1920</b>.</p><button className="secondary" onClick={async()=>{try{await navigator.clipboard.writeText(`${location.origin}/?overlay=1`);setNotice('Overlay URL copied.');}catch{setNotice(`Overlay URL: ${location.origin}/?overlay=1`);}}}><Copy size={15}/>Copy overlay URL</button><a className="text-button" href="/?overlay=1" target="_blank" rel="noreferrer">Open clean overlay <ExternalLink size={13}/></a>{!local&&<small>Use the local app’s overlay URL in OBS to share live scores.</small>}</div>
 ...
 {studioTab==='storage'?<div id="studio-storage"><StorageControls local={local}/></div>:studioTab==='analytics'?<StreamAnalytics local={local}/>:<> <section className="preview-section"><div className="preview-heading"><span><i/> STREAM PREVIEW</span><small>9:16 · 1080 × 1920</small></div><div className="preview-shell">{board}</div><div className="preview-caption"><Check size={14}/>Only the scoreboard appears in your OBS overlay</div></section>
