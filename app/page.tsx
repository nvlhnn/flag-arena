...
export default function Home(){
 ...
 <nav className="studio-tabs" aria-label="Studio controls">{[["live","Live"],["subscribers","Subscribers"],["audio","Audio"],["setup","OBS & health"],["analytics","Analytics"]].map(([id,label])=><button key={id} aria-pressed={studioTab===id} aria-controls={`studio-${id}`} onClick={()=>setStudioTab(id)}>{label}</button>)}</nav>
