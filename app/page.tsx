...
export default function Home(){
 ...
 const layoutControls=(<div className="control-card"><h2>Overlay version</h2><p>Choose the layout shown in your preview and OBS.</p><div className="layout-options"><button className="secondary" aria-pressed={state.overlayLayout!=='superchat'} disabled={busy} onClick={()=>void action('layout',{layout:'current'}).catch(e=>setNotice(e.message))}>Current</button><button className="secondary" aria-pressed={state.overlayLayout==='superchat'} disabled={busy} onClick={()=>void action('layout',{layout:'superchat'}).catch(e=>setNotice(e.message))}>Super Chat</button></div><small>Super Chat: 9 × 11 flags, donors ranked by session total, and a continuous supporter scroll. The layout switches immediately, even before your first donation. $1 USD = 1,000 points in either layout. Subscriber bonus stays +100.</small></div>);
 const scoringControls=(<div className="control-card"><div className="card-title"><h2>Catch-up bonus</h2></div><p>Trailing countries earn up to 5× chat points once the leader reaches 1,000. Subscriber points stay the same.</p><button className="secondary" role="switch" aria-checked={state.catchUpEnabled!==false} disabled={!ready||busy} onClick={async()=>{setBusy(true);try{await action('catch-up',{enabled:state.catchUpEnabled===false});}catch(error){setNotice((error as Error).message);}finally{setBusy(false);}}}>Catch-up bonus: {state.catchUpEnabled!==false?'ON':'OFF'}</button><small>Below 25% of leader: 5× · below 50%: 4× · below 75%: 3× · below 90%: 2× · otherwise: 1×. Applies to new votes.</small></div>);
